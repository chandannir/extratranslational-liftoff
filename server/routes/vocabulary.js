...
router.post('/', requireAuth, upload.single('image'), (req, res) => {
  ...
  const result = db.prepare(`
    INSERT INTO vocabulary (user_id, recording_id, word, label, image_path, language, notes, timestamp)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(req.user.userId, recording_id || null, word, label, image_path, language || 'en', notes, timestamp);
